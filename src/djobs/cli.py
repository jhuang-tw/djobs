...
def _cmd_install_mcp(args: argparse.Namespace) -> None:
    ...
    if not args.full_approve:
        print(
            "\nTip: add --full-approve to include write tools "
            "(enqueue_task, complete_task, fail_task) in autoApprove."
        )
...
def main(argv: list[str] | None = None) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "install-mcp",
        help="Print an mcp.json snippet for VS Code / Claude Desktop",
    )
